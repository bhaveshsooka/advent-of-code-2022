...
def updown(stacks, reps, fromstack, tostack):
    crates = stacks[int(fromstack)][-1*int(reps):]
    [stacks[int(fromstack)].pop() for _ in range(int(reps))]
    print(crates)
    [stacks[int(tostack)].append(crate) for crate in crates]
    return stacks
...
for move in moves_idx:
    print(move)
    stack_lists = updown(stack_lists,move[reps], move[fromidx], move[toidx])
